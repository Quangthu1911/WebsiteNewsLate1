	...
	public String loginadmin(HttpServletRequest request) {
		...
		for (int i = 0; i < list.size(); i++) {
			if (list.get(i).getName().equals(name) && list.get(i).getPassword().equals(pass)) {
				...
				request.setAttribute("newManagement",list1);
...
//		dtnService.save(dtu);
	...
	public String deleteTasks(HttpServletRequest request) throws IOException {
		String id=request.getParameter("id");
		dtnService.delete(Integer.parseInt(id));
		return "admin";
	}
	...
	{
		String id=request.getParameter("id");
		detailnews task=dtnService.findDetailNewsWithID(Integer.parseInt(id));
		request.setAttribute("id", id);
		request.setAttribute("title", task.getTitle());
		request.setAttribute("quote", task.getQuote());
		request.setAttribute("content", task.getContent());
		request.setAttribute("idcategory", task.getIdcategory());
		return "upload_news";
	}
	@PostMapping("/save-update-tasks")
	public String saveup(HttpServletRequest request, @RequestParam("image1") MultipartFile photo) throws NumberFormatException, IOException
	{
		String id=request.getParameter("id");
		detailnews dt=new detailnews();
		dt=dtnService.findDetailNewsWithID(Integer.parseInt(id));
		dt.setContent(request.getParameter("content"));
		dt.setTitle(request.getParameter("title"));
		dt.setIdcategory(Integer.parseInt(request.getParameter("idcategory")) );
		dt.setQuote(request.getParameter("quote"));
		dtnService.save(dt);
		dtnService.updateRecords(photo, Integer.parseInt(id));
		return "admin";
